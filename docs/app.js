...
var createCarousel = function createCarousel(dom) {
  ...
  var slider = new Glider(slides, (_Glider = {
    // `auto` allows automatic responsive
    // width calculations
    slidesToShow: "auto",
    itemWidth: "100%",
    slidesToScroll: "auto"
  }, _defineProperty(_Glider, "itemWidth", "100%"), _defineProperty(_Glider, "itemWidth", "100vw"), _defineProperty(_Glider, "exactWidth", false), _defineProperty(_Glider, "duration", 0.5), _defineProperty(_Glider, "dots", "CSS Selector"), _defineProperty(_Glider, "arrows", {
    prev: prev,
    // may also pass element directly
    next: next
  }), _defineProperty(_Glider, "draggable", false), _defineProperty(_Glider, "dragVelocity", 3.3), _defineProperty(_Glider, "easing", function easing(x, t, b, c, d) {
    return c * (t /= d) * t + b;
  }), _defineProperty(_Glider, "scrollPropagate", false), _defineProperty(_Glider, "eventPropagate", true), _defineProperty(_Glider, "scrollLock", false), _defineProperty(_Glider, "scrollLockDelay", 150), _defineProperty(_Glider, "resizeLock", true), _defineProperty(_Glider, "responsive", [{
    breakpoint: 1200,
    settings: {
      slidesToShow: 4,
      itemWidth: "100%",
...
var Home = function Home() {
  return {
    ...
    view: function view(_ref3) {
      var mdl = _ref3.attrs.mdl;
      return m("article.grid", m("section.row.p-b-25.bg-light", mdl.data.events.any() && m(".glider-contain", SliderController(mdl), m("h2.is-center", "Upcoming Events!"), m(".grouped.#events.glider", mdl.data.events.map(function (event, idx) {
        return m(".card.is-vertical-align m-x-6 row", {
          key: idx,
          style: {
            backgroundImage: event.image,
            alignContent: "stretch"
          }
        }, m("header", event.allDay && m(".tag", "All Day Event!"), m("h5.text-primary text-wrap", event.title)), m("p", m("img", {
          style: {
            maxHeight: "250px"
          },
          src: event.image
      ...
      })), m("nav.nav.p-t-25", m("button.button.nav-left.outline.dark", m(_clarity.AngleDoubleLine, {
        style: {
          transform: "rotate(270deg)"
        }
      })), m("button.button.nav-right.outline.dark", m(_clarity.AngleDoubleLine, {
        style: {
          transform: "rotate(90deg)"
        }
      }))))), m("section.row.p-b-25.bg-primary.text-white", mdl.data.images.any() && m(".glider-contain", SliderController(mdl), m("h2.is-center", "Recent Photos"), m(".grouped.#images.glider", mdl.data.images.map(function (img, idx) {
      ...
      })), m("nav.nav.p-t-25", m("button.button.nav-left.light.outline", m(_clarity.AngleDoubleLine, {
        style: {
          transform: "rotate(270deg)"
        }
      })), m("button.button.nav-right.light.outline", m(_clarity.AngleDoubleLine, {
        style: {
          transform: "rotate(90deg)"
        }
      }))))), m("section.row.p-b-25.bg-light", mdl.data.blogs.any() && m(".glider-contain", SliderController(mdl), m("h2.is-center", "Recent Blogs"), m(".grouped.#blogs.glider", mdl.data.blogs.map(function (blog, idx) {
        return m(".card.opacity-overlay.is-vertical-align m-x-6 row", {
          key: idx
        }, m("header", m("img", {

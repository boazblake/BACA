...
var BlogPreview = {
  view: function view(_ref) {
    ...
    }, "continue reading")), (author == mdl.user.name || mdl.user.isAdmin) && m("footer", m(m.route.Link, {
      selector: "button",
...
var Event = {
  view: function view(_ref) {
    ...
    return m(".modal-container", m(".modal", m("header.modal-header", m("h2.tag", event.title), m(".grouped", m("label", event.startDate), m("label", event.endDate)), m(".grouped", m("label", event.startTime), m("label", event.endTime)), event.allDay && m(".grouped", m("label.tag.primary", "All Day Event")), m(".grouped", m("img", {
      src: event.image
    }))), m("section.modal-content container card", m(".grouped", m("label", event.description))), m("footer.modal-footer", m(".tabs grouped", (event.createdBy == mdl.user.name || mdl.user.isAdmin) && m("button.button.secondary.is-full-width", {
      onclick: function onclick(e) {
        editEvent(true);
        previewEvent(false);
        e.preventDefault();
...
var Home = function Home() {
  return {
    ...
    view: function view(_ref) {
      var mdl = _ref.attrs.mdl;
      return m("article.grid", m("section.row.bg-light", m(".container", m("h2.is-center", "Upcoming Events!"), m(".grouped.overflow-x-auto", mdl.data.events.map(function (event) {
        return m(".card.col-3.is-vertical-align row", {
          style: {
      ...
      })))), m("section.row.bg-primary.text-white", m(".container", m("h2.is-center", "Recent Photos"), m(".grouped.overflow-x-auto", mdl.data.images.map(function (img) {
        return m("img.card.col-3 auto", {
          src: img.thumb,
          style: {// width: "40% !important",
          }
        });
      })))), m("section.row.bg-light", m(".container", m("h2.is-center", "Recent Blogs"), m(".grouped.overflow-x-auto", mdl.data.blogs.map(function (blog) {
        return m(".card.col-3.opacity-overlay.is-vertical-align row", m("figure", m("img", {
          src: blog.img || "images/main.webp"
        })), m("h2", blog.title), m(m.route.Link, {
          selector: "label",
          "class": "tag button bg-primary text-white is-full-width",
          href: "/social/blog-post:".concat(blog.objectId) // style: { opacity: 0 },
          // onmouseover: (e) => (e.target.style.opacity = 1),
          // onmouseleave: (e) => (e.target.style.opacity = 0),
...
var JoinBACA = function JoinBACA(mdl) {
  return {
    view: function view(_ref) {
      var mdl = _ref.attrs.mdl;
      return m("", m("section.bg-light.card", m("hgroup.container", m("p", "BACA operates as a non-profit organization for community and civic improvement and protection and other non-profit purposes."), m("p", "Any income received shall be applied only to the non-profit purposes and objectives of the organization, and no part of the income shall inure to the benefit of any officer or member."), m("p", "We will use the ", m("span.text-primary", "annual $50 membership"), " to maintain ", m(m.route.Link, {
        selector: "a",
        "class": "underline",
        href: "/"
...
m.route(root, "/about", (0, _app["default"])(_index["default"])); // m.route.set("/about")
});

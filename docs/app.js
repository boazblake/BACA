...
var updateNavigationPostion = function updateNavigationPostion(dom, showNav) {
  if (dom) {
    return showNav ? (dom.classList.add("fadeOut"), "unset") : (dom.classList.remove("fadeOut"), "sticky");
  } else {
...
var onLayout = function onLayout(mdl) {
  return function (_ref2) {
    var dom = _ref2.dom;
    log("mdl")(mdl);
    mdl.settings.screenSize == "desktop" && dom.parentNode.addEventListener("scroll", onBodyScroll(mdl));
  };
};
...
var Layout = {
  ...
  view: function view(_ref5) {
    ...
    return m("#layout", {
      ...
      style: {
        position: updateNavigationPostion(state.navDom, mdl.state.showNavMenu())
      },
      "class": vertAlign(mdl)
    }, m(_navbar["default"], {
      mdl: mdl

...
var addResident = function addResident(mdl, state) {
  var onError = function onError(e) {
    var msg = "error finding resident";
    console.log(msg, e);
  };

  var onSuccess = function onSuccess(pin) {
  ...
  (0, _model.findLatLongTask)(mdl, state.input).fork(onError, onSuccess);
};
...
var BonhamAcresMap = function BonhamAcresMap(_ref) {
  ...
  var state = {
    dom: null,
    entities: (0, _model.defaultPushPin)(mdl),
    input: null,
    map: null,
    opts: {
      credentials: "AhqSb9RscePM37EHU7fTFalyXaXPiKCbkxRjYld0ZKrUsOUHzwNCMngoGVgqrht_",
  ...
  (0, _model.loadResidentsTask)(mdl, state).map((0, _model.loadMapConfig)(mdl)).fork(onError, onSuccess);
  return {
    view: function view(_ref2) {
      var mdl = _ref2.attrs.mdl;
      return m("section", ["mod", "admin"].includes(mdl.user.role) && m("section.grouped", m("input", {
        type: "text",
        oninput: function oninput(e) {
        ...
        onclick: function onclick() {
          return addResident(mdl, state);
        }
      }, "Add Resident")), m("section#map", {
        oncreate: function oncreate(_ref3) {
          var dom = _ref3.dom;
...
exports.findLatLongTask = exports.loadResidentsTask = exports.toPushPin = exports.loadMapConfig = exports.setCenterView = exports.getCenterView = exports.defaultPushPin = void 0;
...
var defaultPushPin = function defaultPushPin(mdl) {
  return [toPushPin({
    property: "BACA",
...
exports.defaultPushPin = defaultPushPin;
...
var addEventHandlers = function addEventHandlers(pushPin) {
  var highlight = function highlight(id) {
    console.log(id); // //Highlight the mouse event div to indicate that the event has fired.

    dom.style.background = "LightGreen"; // //Remove the highlighting after a second.
...
var loadResidentsTask = function loadResidentsTask(mdl, state) {
  return mdl.http.back4App.getTask(mdl)("classes/Addresses").map((0, _ramda.prop)("results")).map((0, _ramda.map)(toPushPin)).map((0, _ramda.map)(addEventHandlers)).map(updateStateEntities(state));
};

exports.loadResidentsTask = loadResidentsTask;

var findLatLongTask = function findLatLongTask(mdl, query) {
  return mdl.http.openCage.getLocationTask(mdl)(query).map((0, _ramda.path)(["results", 0])).map((0, _ramda.paths)([["formatted"], ["geometry"]])).chain(function (xs) {
    return xs.includes(undefined) ? _data["default"].rejected() : _data["default"].of(xs);
  }).map(function (_ref2) {
...
exports.findLatLongTask = findLatLongTask;
});

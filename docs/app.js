...
var Comments = {
  view: function view(_ref2) {
    var _ref2$attrs = _ref2.attrs,
        ...
        mdl = _ref2$attrs.mdl;
    return m("section.container.p-y-25", mdl.state.isAuth() && m("aside.card", m("header.row", m("figure.col-2", m("img.avatar", {
      style: {
        maxWidth: "100px"
      },
      src: mdl.user.avatar || _Utils.AVATAR_URL
    }), m("caption", mdl.user.name)), m(".col-10", m("textarea.w100", {
      // style: { width: "80%" },
      rows: 3,
      onchange: function onchange(e) {
...
window.location.hash ? m.route.set(format(window.location.hash)) : m.route.set("/about");
});
